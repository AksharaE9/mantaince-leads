...
export const downloadDeliveryDataTemplate = async (req, res) => {
    ...
    try {
        ...
        const sampleValues = {
            ...
            deliveryDate: '05-08-2026', deliveryTime: '2:00 PM - 3:00 PM',
        };
...
/**
 * POST /delivery-data/inspect-sheets
 *
 ...
 */
export const inspectDeliveryDataSheets = async (req, res) => {
    const file = req.file;
    try {
        if (!file) return operationError(res, { code: ErrorCodes.MISSING_REQUIRED_FIELD, message: 'A file is required', section: 'delivery_data', operation: 'inspect_sheets', field: 'file' });
        ...
        return sendControllerError(res, error, 'inspectDeliveryDataSheets');
    }
};
...
export const uploadDeliveryDataCsv = async (req, res) => {
    const { verticalId, columnMapping } = req.body;
    const file = req.file;
    ...
    try {
        ...
        if (process.env.VERCEL) {
            ...
            const logRes = await query(`
                INSERT INTO csv_upload_logs (id, uploaded_by, vertical_id, file_name, original_file_name, status, entity_type, sheet_indices, column_mapping)
                VALUES ($1, $2, $3, $4, $5, 'processing', 'delivery_data', $6, $7)
                RETURNING *
            `, [logId, req.user.sub, verticalId, fileName, file.originalname, JSON.stringify(sheetIndices), parsedColumnMapping ? JSON.stringify(parsedColumnMapping) : null]);
        ...
        const logRes = await query(`
            INSERT INTO csv_upload_logs (id, uploaded_by, vertical_id, file_name, original_file_name, status, entity_type, sheet_indices, column_mapping)
            VALUES ($1, $2, $3, $4, $5, 'queued', 'delivery_data', $6, $7)
            RETURNING *
        `, [logId, req.user.sub, verticalId, fileName, file.originalname, JSON.stringify(sheetIndices), parsedColumnMapping ? JSON.stringify(parsedColumnMapping) : null]);
